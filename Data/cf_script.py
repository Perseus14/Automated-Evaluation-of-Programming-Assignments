...
for submission in submissions:
    if( (submission['verdict'] == u'OK' or submission['verdict'] == u'WRONG_ANSWER') and (submission['programmingLanguage'] in LANG) and (submission['problem']['index'] == 'A')):
        ...
        if(submission['verdict'] == u'OK'):
            new_directory = parent_dir + '/' + str(con_id) + '/Correct'
            soln_id_c+=1
            soln_id = soln_id_c
        else:
            new_directory = parent_dir + '/' + str(con_id) + '/Incorrect'
            soln_id_ic+=1
            soln_id = soln_id_ic 
        if not os.path.exists(new_directory):
            os.makedirs(new_directory)
        filename = new_directory + '/' + prob_id + str(soln_id) +'.' + ext 
