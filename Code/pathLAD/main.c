...
bool solve(int timeLimit, bool firstSol, bool induced, int verbose, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
    ...
    if (nextVar==-1){// All vertices are matched => Solution found
        ...
        if (verbose >= 1){
            printf("Solution %d: \n",nbSol);
            for (u=0; u<Gp->nbVertices; u++) printf("%d=%d \n",u,D->val[D->firstVal[u]]);
            printf("\n");
        }
        resetToFilter(D,Gp->nbVertices);
        return true;
...
int main(int argc, char* argv[]){
	...
	{
		//printf("-------------------------- \n %d %d %d %d \n", &Gp->nbVertices+nbIsolatedP, &Gt->nbVertices, &Gp->maxDegree, &Gt->maxDegree);
        return printStats(false);
	}
	// Initialize domains
	Tdomain *D = createDomains(Gp, Gt);
	//printf("-------------------------- \n %d %d \n", &Gp->nbVertices, &Gt->nbVertices);
	if (!initDomains(induced, D, Gp, Gt)){ //printf("-------------------------- \n %d %d \n", &Gp->nbVertices, &Gt->nbVertices); 
	return printStats(false);}
